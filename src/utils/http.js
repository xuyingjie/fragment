...
// or use Promise
export function get({ key, passwd, iv, buf, progress }) {
    ...
    xhr.onload = () => {
      if (xhr.readyState === 4) {
        if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 304) {
          ...
          crypto.decrypt(passwd, iv, xhr.response)
            .then(out => {
              if (buf) {
                resolve(out)
              } else {
                let str = crypto.arrayBufferToStr(out)
                resolve(str)
              }
            })
...
export function upload({ key, data, passwd, buf, progress }) {
  let promise = new Promise(resolve => {
